	...
	)
{
	...
	char* pFileName;
	GetFullPathNameA(progname, MAX_PATH, full_path, &pFileName);
	...
	int result = OsraProcessImage(
		input_file_option.getValue().c_str(),
		output_file_option.getValue().c_str(),
		...
		output_format_option.getValue().c_str(),
		embedded_format_option.getValue().c_str(),
		...
		osra_dir.c_str(),
		spelling_file_option.getValue().c_str(),
		superatom_file_option.getValue().c_str(),
